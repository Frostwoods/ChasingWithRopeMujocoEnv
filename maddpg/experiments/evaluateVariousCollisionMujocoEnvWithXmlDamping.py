...
from environment.mujocoEnv.multiAgentMujocoEnv import ResetUniformWithoutXPos,ResetFixWithoutXPos
...
class TransitionFunctionWithoutXPos:
    ...
    def __call__(self, state, actions):
        ...
        for simulationFrame in range(self.numSimulationFrames):
            ...
            self.simulation.data.qvel[:]=np.array([ agentNewQVel(agentIndex) for agentIndex in range(self.numAgents)]).flatten()
...
def simuliateOneParameter(parameterDict,evalNum,randomSeed,dt):
    ...
    damping=parameterDict['damfrom matplotlib import pyplot as pltping']
...
#load env xml and change some geoms' property
    ...
    if saveTraj:
        ...
        trajectoriesSaveDirectory=trajSavePath = os.path.join(dirName,'..', 'trajectory','variousCollsionWithoutSpeedLimit','dt={}'.format(dt))
...
def main():
    ...
    manipulatedVariables['damping'] =[2.5]
   ...
   # dampratio：0.3-0.8
    ...
    trajectoryDirectory = os.path.join(dataFolderName, 'trajectory','variousCollsionWithoutSpeedLimit','dt={}'.format(dt))
    ...
    originEnvTrajSavePath =os.path.join(dirName,'..','trajectory','variousCollsionWithoutSpeedLimit','collisionMoveOriginBaseLine_evalNum={}_randomSeed={}.pickle'.format(evalNum,randomSeed))
