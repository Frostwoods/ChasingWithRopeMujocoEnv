...
def main():
    ...
    if debug:
        ...
        numMasters = int(condition['numMasters'])
        saveTraj = True
        visualizeTraj = False
        maxTimeStep = 50
    ...
    physicsDynamicsPath=os.path.join(dirName,'..','..','environment','mujocoEnv','rope','leasedNewWithoutMasterControl.xml')
    ...
    reshapeActionList = [ReshapeAction(5),ReshapeAction(5),ReshapeAction(2.5)]
    ...
    modelFolder = os.path.join(dirName, '..', 'trainedModels', 'mujocoMADDPGLeasedWithoutMasterControl','damping={}_frictionloss={}'.format(damping,frictionloss))
    ...
    # visualize
    if visualizeTraj:
        entitiesColorList = [wolfColor] * numWolves + [sheepColor] * numSheeps + [blockColor] * numMasters
